...
%token INT CHAR WHILE IF ELSE RETURN COMMA ASSIGN PLUS MINUS DIV MUL SEMI LEFT_CURLY RIGHT_CURLY LEFT_BRAC RIGHT_BRAC LEFT_SQBRAC RIGHT_SQBRAC GREATER_THAN LESS_THAN COMPARE_EQUAL LESS_THAN_EQUAL GREATER_THAN_EQUAL COMPARE_NOT_EQUAL AND
%union{
    int num;
    char *str;
}
...
func_call : ident LEFT_BRAC RIGHT_BRAC SEMI
    | ident LEFT_BRAC param RIGHT_BRAC SEMI
;
...
array : ident LEFT_SQBRAC expr RIGHT_SQBRAC {fprintf(yyout,"LEFT_SQBRAC : [ \n"); fprintf(yyout,"RIGHT_SQBRAC : ] \n");}
;

expr : LEFT_BRAC expr RIGHT_BRAC
    | expr ASSIGN expr          {fprintf(yyout,"ASSIGN : = \n");}
    ...
    | expr AND expr             {fprintf(yyout,"Operator : AND \n");}
    | array
    | number                    
    | ident
;
